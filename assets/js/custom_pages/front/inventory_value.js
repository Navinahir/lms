...
function setDataTable(date, user) {
    ...
    var table = $('.datatable-responsive-control-right').dataTable({
        ...
        order: [[0, "desc"]],
        ajax: site_url + 'reports/get_inventory_value_items_data',
        responsive: {
            details: {
                type: 'column',
                target: -1
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                    if(full.inventory_value > 0)
                    {
                        var inv_val = parseFloat(full.inventory_value).toFixed(2);
                        x=inv_val.toString();
                        var afterPoint = '';
                        if(x.indexOf('.') > 0)
                        ...
                        if(otherNumbers != '')
                            lastThree = ',' + lastThree;
                        var inventory_value = otherNumbers.replace(/\B(?=(\d{2})+(?!\d))/g, ",") + lastThree + afterPoint;
                    } else {
...
$(document).on('click', '.pdf_button', function (e) {
    ...
    var user = $('#user').val();
    var url = site_url + 'reports/print_inventory_value_data';
    if (user != null) {
