...
var table = $('.datatable-responsive-control-right').dataTable({
    ...
    order: [[2, "desc"]],
    ajax: site_url + 'estimates/get_items_data_trash',
    responsive: {
        details: {
            type: 'column',
            target: -1
    ...
    columns: [
        ...
        },
        {
            ...
            render: function (data, type, full, meta) {
                action = '';
                action += '<a href="' + site_url + 'estimates/trash_recover/' + btoa(full.id) + '" class="btn custom_dt_action_button btn-xs" onclick="return confirm_recover_alert(this)" title="Recover">Recover</a>';
                action += '<a href="' + site_url + 'estimates/trash_delete/' + btoa(full.id) + '" class="btn custom_dt_action_button btn-xs" onclick="return confirm_delete_alert(this)" title="Delete">Delete</a>';
                return action;
            },
            sortable: false,
        },
...
$(document).on('click','.custom_recover_button',function(){
    ...
    if(recover_id != "")
    {
        swal({
            ...
            $.ajax({
                url: 'Estimates/recover_multiple',
                method: 'POST',
                ...
                success: function(data){
                    if(data == 0) {
                        ...
                        window.location.href = site_url + 'estimates';
                    }
                },
            });
        });
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data == 0) {
                        ...
                        window.location.href = site_url + 'estimates/estimate_trash';
                    }
                },
            });
        });
...
function confirm_recover_alert(e) {
    swal({
        title: "Are you sure?",
        text: "Are You sure you want to recover this estimate?",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#FF7043",
        confirmButtonText: "Yes, Recover it!"
