$('.datatable-basic').dataTable({
    ...
    order: [[0, "asc"]],
    ajax: site_url + 'roles/get_roles_ajax_data',
    columns: [
        {
            data: "sr_no",
            visible: true,
        ...
        },
        {
            ...
            render: function (data, type, full, meta) {
                ...
                if (full.id != 4) {
                    if (edit == 1) {
                        action += '<a href="' + site_url + 'users/roles/edit/' + btoa(full.id) + '" class="btn custom_dt_action_button btn-xs" title="Edit">Edit</a>';
                    }
                    if (dlt == 1) {
                        action += '&nbsp;&nbsp;<a href="' + site_url + 'roles/action/delete/' + btoa(full.id) + '" class="btn custom_dt_action_button btn-xs" data-id="' + full.id + '" onclick="return confirm_alert(this,\'Delete\')" title="Delete">Delete</a>';
                    }
                }
                return action;
            },
...
if (add == 1) {
    var add_button = '<div class="text-right"><a href="' + site_url + 'users/roles/add" class="btn bg-teal-400 btn-labeled custom_add_button"><b><i class="icon-plus-circle2"></i></b> Add Role</a></div>';
    ...
    swal({
        ...
        function (isConfirm) {
            if (isConfirm) {
                ...
                $.ajax({
                    type: 'POST',
                    url: site_url + 'roles/count_total_user',
                    async: false,
                    dataType: 'JSON',
                    data: { id: id },
