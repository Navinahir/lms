...
$(document).ready(function () {
    ...
    var estimate_id = 0;
    if(estimate_notification == 1)
    {
        notification("Estimate added" ,"Khyati Mehta", "04-11-2019", "1");
        $.post(site_url + 'home/estimate_notification', function(data) {
        });
    }
    ...
    $(document).on('click', '.estimate-action', function () {
        ...
        $("#hidden_submit_value").empty().html('<input type="hidden" name="save_send" value="' + button_val + '" />');
        $(this).trigger('submit');
    });
    ...
    var redirect_status = print_preview_url;
    if(redirect_status != "" && redirect_status != null)
    {
        window.location.replace(redirect_status); 
    }
...
var table = $('.datatable-responsive-control-right').dataTable({
    ...
    order: [[0, "desc"]],
    ajax: site_url + 'estimates/get_items_data',
    responsive: {
        details: {
            type: 'column',
            target: -1
 ...
 /*       {
        ...
        },
        {
            ...
            render: function (data, type, full, meta) {
                ...
                action += '<a target="_BLANK" href="' + site_url + 'estimates/print_pdf/' + btoa(full.id) + '" class="btn btn-xs custom_dt_action_button" title="Print">Print</a>';
    ...
    footerCallback: function (row, data, start, end, display) {
        var api = this.api();
        api.columns('.estimate_sum', {page: 'current'}).every(function () {
            var sum = this
                    .data()
                    .reduce(function (a, b) {
...
if (add == 1) {
    var add_button = '<div class="text-right"><a href="' + site_url + 'estimates/add" class="btn bg-teal-400 btn-labeled custom_add_button"><b><i class="icon-plus-circle2"></i></b> Add Estimate</a></div>';
...
function confirm_alert(e) {
    swal({
        title: "Are you sure?",
        text: "Estimate will move under the trash.",
        type: "warning",
...
/**********************************************************
 Form Validation
 ***********************************************************/
var validator = $("#add_estimate_form").validate({
...
function add_html(num) {
    ...
    var html = "<tr class='" + estimate_id + "' id='tr_" + num + "' data-value='" + num + "' data-part_tax_id='" + num + "'>" +
            ...
            " </select>" +
            "</td>" +
            "<td class='select_part estimates-select2' id='td_part_no_" + num + "'>" +
...
function add_service_html(num) {
    var html = "<tr class='service_" + estimate_id + "' id='service_tr_" + num + "' data-value='" + num + "' data-service_tax_id ='"+ num + "'>" + "<td class='select_service' id='td_service_no_" + num + "'>" +
            ...
            "<td class='' id='td_service_note_" + num + "'>"+
                "<input type='text' class='form-control service_note' name='service_note[]' value='' placeholder = 'Service note' autocomplete='off' />"+
            "</td>"+
            ...
            "<select data-placeholder='Select a Tax...' class='service_tax_multiple select select-size-sm select-service-tax-data service_select_multiple' multiple='multiple' id='service_tax_id_" + num + "' data-service_tax_id ='"+ num + "' name='service_tax_id[service_tax_id_" + num + "][]'>" +
            "</select>" +
            ...
            "<td id='td_service_remove_" + num + "'><span class='service_remove' id='service_remove_" + num + "'><i class='icon-trash'></i></span></td>" +
            "</tr>";
        setTimeout(function () {
...
$(document).on('click', '.td_service_rate', function () {
    ...
    if ($('#td_service_no_' + num).hasClass('select_service') == false) {
        var html = '<input type="text" value="' + rate + '" id="input_service_rate_' + num + '" data-service_tax_id ="'+ num + '" class="service_' + estimate_id + ' input_service_rate form-control input-xs"/>';
...
// $('#email_id').val(data).empty();
// $.each(data, function(key, value) {   
//     $('#email_id').append($("<option></option>").attr("value", key).text(data.email)); 
// });
...
// Change Draft and estimate status
