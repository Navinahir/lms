...
setTimeout(function () {
    ...
    setInvoiceTaxDataTable(date, tax);
    setServiceTaxDataTable(date, tax);
}, 500);
...
//        dom: '<"datatable-header"fl><"datatable-scroll"t><"datatable-footer"ip>',
        ...
        columns: [
            ...
            //     render: function (data, type, full, meta) {
            //         var action = '';
            //         action += '<a href="javascript:void(0);" class="btn btn-xs custom_dt_action_button invoice_view_btn" title="View" id="' + btoa(full.id) + '">View</a>';
            //         return action;
            //     },
            //     sortable: false,
            // },
    ...
    var pdf_button = '&nbsp;<button type="button" class="btn bg-teal-400 btn-labeled pdf_button"><b><i class="icon-printer"></i></b> Print</button>';
    $('.dt-buttons').append(pdf_button);
}

$(document).on('click','.invoice_view_btn',function(){
    var id = $(this).attr('id');
    var date = $('#date_range').val();
    var tax = $('#tax').val();
    $.ajax({
        url: 'Reports/invoice_list',
        type: 'POST',
        data: {id: id,date: date,tax: tax},
        success: function(response){
            $('#invoice_view_body').html(response);
            $('#invoice_view_modal').modal('show');
        },
    });
});

function setInvoiceTaxDataTable(date, tax) {
    $(document).find('.datatable-responsive-invoices-tax').DataTable().clear().draw();
    $(document).find('.datatable-responsive-invoices-tax').DataTable().destroy();
    $('.datatable-header').append('');

    var table_invoice = $('.datatable-responsive-invoices-tax').dataTable({
        dom: 'lBfrtip',
//        dom: '<"datatable-header"fl><"datatable-scroll"t><"datatable-footer"ip>',
        autoWidth: false,
        processing: true,
        ...
        ajax: {
            'type': 'GET',
            "url": site_url + 'reports/get_invoice_tax_data',
            "data": {
                "date": date,
                "tax": tax
            },
        ...
        columns: [
            ...
            {
                data: "invoice_tax_amount",
                visible: true,
                className: "invoice-tax-sum",
                render: function (data, type, full, meta) {
        ...
        footerCallback: function (row, data, start, end, display) {
            var api = this.api();
            var texTotal = api.column(3, {page: 'current'}).data().sum();
            texTotal = parseFloat(texTotal).toFixed(2);
            $(api.column(3).footer()).html(texTotal);

            var invTotal = api.column(4, {page: 'current'}).data().sum();
            invTotal = parseFloat(invTotal).toFixed(2);
            $(api.column(4).footer()).html(invTotal);
        },
...
function setServiceTaxDataTable(date, tax) {
    $(document).find('.datatable-responsive-service-tax').DataTable().clear().draw();
    $(document).find('.datatable-responsive-service-tax').DataTable().destroy();
    $('.datatable-header').append('');

    var table_invoice = $('.datatable-responsive-service-tax').dataTable({
        dom: 'lBfrtip',
//        dom: '<"datatable-header"fl><"datatable-scroll"t><"datatable-footer"ip>',
        autoWidth: false,
        processing: true,
        ...
        ajax: {
            'type': 'GET',
            "url": site_url + 'reports/get_service_tax_data',
            "data": {
                "date": date,
                "tax": tax
            },
