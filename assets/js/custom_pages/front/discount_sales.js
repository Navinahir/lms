...
var table = $('.datatable-responsive-control-right-part').dataTable({
    ...
    ajax: site_url + 'reports/get_part_discount_sales/'+from_date+to_date,
    responsive: {
        details: {
            type: 'column',
            target: -1
    ...
    columns: [
        ...
        },
        {
            ...
            visible: true,
            render: function (data, type, full, meta) {
                return parseFloat(full.discount_rate).toFixed(2);
            }
        },
        {
            data: 'responsive',
    ...
    "fnDrawCallback": function (row, data, start, end, display) {
        ...
        var total_discount = api.column(5, {page: 'current'}).data().sum();
        total_discount = parseFloat(total_discount).toFixed(2);
        $(api.column(5).footer()).html(total_discount);
    }
});
...
var table = $('.datatable-responsive-control-right-service').dataTable({
    ...
    order: [[1, "desc"]],
    lengthMenu: [[10, 20, 30, 50, -1], [10, 20, 30, 50, "All"]],
    ajax: site_url + 'reports/get_service_discount_sales/'+from_date+to_date,
    responsive: {
        details: {
            type: 'column',
            target: -1
...
$(document).on('click', '.part_print', function (e) {
    ...
    var url = updateQueryStringParameter(site_url + 'reports/print_part_discount_sales', "date", date);
...
$(document).on('click', '.service_print', function (e) {
    ...
    var date = from_date+to_date;
    var url = updateQueryStringParameter(site_url + 'reports/print_service_discount_sales', "date", date);
    window.location.href = encodeURI(url);
});
