...
$(function () {
    $('.datatable-basic').dataTable({
        ...
        order: [[1, "asc"]],
        ajax: site_url + 'admin/equipments/get_manufacturer',
        columns: [
            {
                data: "sr_no",
                sortable: false,
            ...
            },
            {
                ...
                render: function (data, type, full, meta) {
                    ...
                    action += '&nbsp;&nbsp;<a href="' + site_url + 'admin/equipments/manufacturer/delete/' + btoa(full.id) + '" class="btn custom_dt_action_button btn-xs" onclick="return confirm_alert(this)" title="Delete">Delete</a>';
                    return action;
                },
                sortable: false,
            }
...
$(document).on('click', '.edit', function () {
    ...
    var url = base_url + 'admin/equipments/get_manufacturer_by_id';
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#txt_name').focus();
            $('#txt_manufacturer_id').val(data.id);
            $("#txt_name").rules("add", {
                remote: site_url + "admin/equipments/checkUnique_Manufacturer_Name/" + data.id,
                messages: {
                    remote: $.validator.format("This name already exist!")
                }
            });
            $("#add_manufacturer_form").validate().resetForm();
        }
    });
});
...
function cancel_click() {
    ...
    $('#manufacturers_form_row').css('z-index', '0');
    $('.bulk_upload_div').removeClass('hide');
    $('#txt_name').val('');
    $('#txt_description').val('');
    $('#txt_manufacturer_id').val('');
    $("#txt_name").rules("add", {
...
var validator = $("#add_manufacturer_form").validate({
