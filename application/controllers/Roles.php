...
class Roles extends MY_Controller {
    ...
    public function display_roles() {
        ...
        $this->template->load('default_front', 'front/users/role_display', $data);
    }
     ...
     * @author HPA [Last Edited : 01/06/2018]
     */
    public function get_roles_ajax_data() {
        $final['recordsTotal'] = $this->users_model->get_roles_ajax_data('count');
        $final['redraw'] = 1;
        $final['recordsFiltered'] = $final['recordsTotal'];
        $roles = $this->users_model->get_roles_ajax_data('result');
    ...
    public function edit_roles($id = null) {
        $data['title'] = 'Add Role';
        $data['permissions'] = $this->users_model->get_controllers();
        $data['access'] = '';
        if (!is_null($id)) {
            $record_id = base64_decode($id);
            $data['title'] = 'Edit Role';
            $data['dataArr'] = $this->users_model->get_all_details(TBL_ROLES, array('id' => $record_id))->row_array();
            $data['access'] = $this->users_model->get_role_permission($record_id);
        }
        $exist_permission = [31, 44, 45, 48, 54, 83, 60, 61, 65];
        ...
        $this->template->load('default_front', 'front/users/add_role', $data);
    }
