    ...
    <table style="padding:5px; width: 100%">
        <tr>
            <td width="115" style="padding: 15px;">
                ...
                <p style="font-size: 20px;">$<?= number_format($itemArr['retail_price'], 2) ?></p>
            </td>
            <td style="border-left: solid 5px #000; padding: 0 0 0 10px;">
                <p style="font-size: 20px; font-weight: bold;">Item Part No:</p>
                <p style="font-size: 20px;"><?= $itemArr['part_no']; ?></p>
                <p style="font-size: 20px; font-weight: bold;">Item Description:</p>
                <?php $count_desc = strlen($itemArr['description']); ?>
                <?php 
                    if($count_desc > 30) {
                ?>
                    <p style="font-size: 20px;"><?= substr($itemArr['description'],0,30).'...'; ?></p>
                <?php }  else { ?>
                    <p style="font-size: 20px;"><?= substr($itemArr['description'],0,30); ?></p>
                <?php } ?>
                <p style="font-size: 20px; font-weight: bold;">Printed Label Date:</p>
                <p style="font-size: 20px;"><?= date('F dS,Y'); ?></p>
            </td>
        </tr>
    </table>
</div>