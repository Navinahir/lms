<div class="row">
    ...
    <div class="col-lg-12">
        <div class='panel border-blue location_main'>
            ...
            <div class='panel-body'>
                <div class="labl-btn-wrap">
                    <a href='<?php echo base_url() . 'estimates/add?id=' . base64_encode($viewArr['id']) . '&location_id=' . base64_encode($location_id) . $transponder . '&drop_status=' . base64_encode(1)?>' class='btn bg-pink-300 mb-3'>Add to Estimate</a>
                    
                    <?php if($est_inv_status > 0 && $this->session->userdata('u_location_id') != "") { ?>
                        <a href='<?php echo base_url() . 'invoices/add?id=' . base64_encode($viewArr['id']) . '&location_id=' . base64_encode($location_id) . $transponder . '&drop_status=' . base64_encode(1)?>' class='btn bg-indigo-300 mb-3'>Add to Invoice</a>
                    <?php } else if ($viewArr['total_quantity'] > 0 && $this->session->userdata('u_location_id') == "") { ?>
                        <a href='<?php echo base_url() . 'invoices/add?id=' . base64_encode($viewArr['id']) . '&location_id=' . base64_encode($location_id) . $transponder . '&drop_status=' . base64_encode(1)?>' class='btn bg-indigo-300 mb-3'>Add to Invoice</a>
                    <?php } else { ?>
                    <?php } ?>
                    
                    <a href='<?php echo base_url() . 'receive_inventory/'.base64_encode($viewArr['id']); ?>' class='btn btn-success mb-3'>Receive Inventory</a>
                    <a href='<?php echo base_url() . 'adjust_inventory/'.base64_encode($viewArr['id']); ?>' class='btn btn-info mb-3'>Adjust Inventory</a>
                    <a href='<?php echo base_url() . 'move_inventory/'.base64_encode($viewArr['id']); ?>' class='btn btn-warning mb-3'>Move Inventory</a>
                </div>
            </div>
        </div>
    </div>
